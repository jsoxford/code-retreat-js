...
exports.run = function(fileToWatch) {
    ...
    fs.watch(fileToWatch, function(event, filename){
        if(event == 'change' && !fsTimeout){
            fsTimeout = setTimeout(function() { fsTimeout=null }, 1500) // give 1.5 seconds for multiple events
            loadAndTest(fileToWatch);
        }
    });
}
...
function connectToStatsBucket(){
    ...
    client.on('close', function() {
        if(!connRetryTimeout){
            connRetryTimeout = setTimeout(function() { connRetryTimeout = null; connectToStatsBucket() }, 5000);
        }
    });
      ...
      try{
          switch(payloadObj.action){
            case "processIteration":
              response.payload = [
                {
                    generation: payloadObj.payload.generation,
                    result: payloadObj.payload.result
                },{
                    generation: payloadObj.payload.generation+1,
                    result: session.tick(payloadObj.payload.result)
                }
              ];
              break;
            default:
