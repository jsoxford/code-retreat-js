...
exports.run = function (fileToWatch) {
  ...
  watchedFileChecksum = checksum(fileToWatch);

  setInterval(function () {
    var newChecksum = checksum(fileToWatch);
    if (newChecksum != watchedFileChecksum) {
      loadAndTest(fileToWatch);
      watchedFileChecksum = newChecksum;
    }
  }, 1000);
};
...
function checksum(filename) {
  var algorithm = 'md5';
  return require('crypto').createHash(algorithm).update(fs.readFileSync(filename)).digest('base64');
}
  ...
  try {
    switch (payload.action) {
      ...
      case "tickCell":
        response.payload = {
          generation: payload.payload.generation,
          lives: session.tickCell(payload.payload.result),
          from: payload.payload.result,
          x: payload.payload.x,
          y: payload.payload.y
        };
        break;
